...
Route::view('/multilogin', 'auth.multilogin')->name('multilogin');
...
Route::prefix('admin')->name('admin.')->group(function () {
    // ログイン
    Route::get('/login', [LoginController::class, 'showLoginForm'])->name('login.form'); // GET フォーム
    Route::post('/login', [LoginController::class, 'login'])->name('login');             // POST 認証
    ...
    Route::resource('owners', AdminOwnerController::class); // 管理者が店舗代表者を管理
    // CSVアップロード専用のルート
    Route::get('upload-csv', [CsvImportController::class, 'showUploadForm'])->name('upload_csv');
    Route::post('upload-csv', [CsvImportController::class, 'importCsv'])->name('upload_csv');
});

// * Owner
Route::prefix('owner')->name('owner.')->group(function () {
    // Route::get('/login', [OwnerLoginController::class, 'showLoginForm'])->name('login.form'); // GET
